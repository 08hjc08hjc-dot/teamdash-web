    ...
    <div>
      ...
      <div className="bg-td-card backdrop-blur-xl rounded-b-2xl border border-t-0 border-td-border p-4 sm:p-6 mb-6">
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            {editing ? (
              <input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter') { if (titleVal.trim()) { updateProject(project.id, { title: titleVal.trim(), description: descVal }); } setEditing(false); } if (e.key === 'Escape') setEditing(false); }}
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button
              onClick={() => {
                if (editing) {
                  if (titleVal.trim()) updateProject(project.id, { title: titleVal.trim(), description: descVal });
                  setEditing(false);
                } else {
              ...
              className={`p-2 rounded-xl transition-colors ${editing ? 'text-teal-400 hover:bg-teal-500/10' : 'text-td-text-muted hover:bg-td-hover'}`}
            >
        ...
        {editing ? (
          ...
          project.description && <p className="text-base text-td-text-muted mt-2">{project.description}</p>
        )}
        ...
        <div className="flex gap-2 mt-4 flex-wrap">
          {(['todo', 'in_progress', 'done'] as const).map((status) => {
            ...
            return (
              <span key={status} className="text-sm px-3 py-1 rounded-full font-medium" style={{ backgroundColor: STATUS_COLORS[status] + '50', color: STATUS_COLORS[status] }}>
            ...
            return (
              <Link href={`/tasks/${task.id}`} key={task.id} className="block bg-td-card backdrop-blur-xl rounded-xl p-3 sm:p-4 border border-td-border hover:bg-td-hover-strong transition-all">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: STATUS_COLORS[task.status] + '55', color: STATUS_COLORS[task.status] }}>
                    {TASK_STATUS_LABELS[task.status]}
                  </span>
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: PRIORITY_COLORS[task.priority] + '55', color: PRIORITY_COLORS[task.priority] }}>
                  ...
                  {taskAssignees.length > 0 && (
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs text-td-text-secondary">
                        {taskAssignees.length <= 2 ? taskAssignees.map((a) => a.name.split(' ')[0]).join(', ') : `${taskAssignees[0].name.split(' ')[0]} 외 ${taskAssignees.length - 1}명`}
                      </span>
                    </div>
                  )}
                  {task.dueDate && (
