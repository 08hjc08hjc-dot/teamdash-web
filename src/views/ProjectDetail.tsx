      ...
      <div className="bg-td-card backdrop-blur-xl rounded-b-2xl border border-t-0 border-td-border p-4 sm:p-6 mb-6">
        ...
        <div className="flex gap-2 mt-4 flex-wrap">
          {(['todo', 'in_progress', 'done'] as const).map((status) => {
            ...
            return (
              <span key={status} className="text-sm px-3 py-1 rounded-full font-medium" style={{ backgroundColor: STATUS_COLORS[status] + '70', color: STATUS_COLORS[status] }}>
            ...
            return (
              <Link href={`/tasks/${task.id}`} key={task.id} className="block bg-td-card backdrop-blur-xl rounded-xl p-3 sm:p-4 border border-td-border hover:bg-td-hover-strong transition-all">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: STATUS_COLORS[task.status] + '75', color: STATUS_COLORS[task.status] }}>
                    {TASK_STATUS_LABELS[task.status]}
                  </span>
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: PRIORITY_COLORS[task.priority] + '75', color: PRIORITY_COLORS[task.priority] }}>
